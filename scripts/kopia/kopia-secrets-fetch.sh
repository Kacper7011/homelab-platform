...
# Move this scipt to /usr/local/bin and make it executable. 
# It will fetch secrets from Vault and write them to /run/kopia-env for use by Kopia.

set -e

VAULT_ADDR="http://10.10.10.216:8200"
ROLE_ID="$(cat ~/.vault/kopia/role_id)"
SECRET_ID="$(cat ~/.vault/kopia/secret_id)"
...
SECRETS=$(curl -sf \
  ...
  $VAULT_ADDR/v1/secret/data/services/kopia)

ACCESS_KEY=$(echo $SECRETS | jq -r '.data.data.AWS_ACCESS_KEY_ID')
SECRET_KEY=$(echo $SECRETS | jq -r '.data.data.AWS_SECRET_ACCESS_KEY')
